...
class NotInterestingConsumerService:
    """
    OOP consumer service for the 'not_interesting' topic.
    ...
    def __init__(self) -> None:
        ...
        """
        logger.info("Initializing NotInterestingConsumerService")
        
        try:
            self._consumer = self._create_consumer()
            self._client, self._coll = self._setup_mongodb()
            logger.info("NotInterestingConsumerService initialized successfully")
            
        except Exception as e:
            logger.error("Failed to initialize NotInterestingConsumerService: %s", str(e), exc_info=True)
            raise
