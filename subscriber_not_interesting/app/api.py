...
# Initialize consumer service with proper error handling
try:
    _service = NotInterestingConsumerService()
    logger.info("Not-interesting subscriber service initialized successfully")
except Exception as e:
    logger.error("Failed to initialize Not-interesting subscriber service: %s", str(e), exc_info=True)
    _service = None
...
def health() -> Dict[str, str]:
    ...
    try:
        ...
        return {"status": "healthy", "message": "Not-interesting subscriber service is running"}
...
def shutdown_event():
    ...
    """
    logger.info("Shutting down not-interesting subscriber service")
    if _service:
        _service.close()
